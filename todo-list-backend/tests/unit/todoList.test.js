...
describe('Testa se o nome do banco é StorageManager', () => {
  it('conecta do StoreManager', async () => {
...
/* describe('Products', () => {
  describe('Insere um novo produto no BD', () => {
    describe('quando é inserido com sucesso', () => {
      it('retorna um objeto', async () => {
        const response = await todoListModel.createProduct('Flavio', 8000);
  ...
  describe('testa a busca de produtos pelo nome', () => {
    it('quando retorna o nome correto', async () => {
      await todoListModel.createProduct('Flavio', 8000);
      const { name } = await todoListModel.getProductByName('Flavio');
  ...
  describe('Testa funcao updateProduct', () => {
    it('testa se atualizou', async () => {
      const { _id: id } = await todoListModel.createProduct('Flavio', 8000);
      await todoListModel.updateProduct(id, 'Jao', 5);
      const { quantity } = await todoListModel.getProductById(id);
  ...
  describe('testa funcao delete', () => {
    it('testa se deleta corretamente', async () => {
      const { _id: id } = await todoListModel.createProduct('Flavio', 8000);
      await todoListModel.deleteProduct(id);
      const response = await todoListModel.getProductById(id);
  ...
  describe('Obtem lista completa de produtos, getAll()', () => {
    it('retorna um array', async () => {
      const response = await todoListModel.getAll();
  ...
  describe('Obtem produtos por id, getProductById()', () => {
    describe('Quando o id não é válido', () => {
      it('retorna null', async () => {
        const response = await todoListModel.getProductById('123');
